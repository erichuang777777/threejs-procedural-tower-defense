...
import { LEVELS, levelById } from './data/maps.js';
import { PROMOTION_TIERS, TALENTS } from './data/promotions.js';
import { buildMap, gridToWorld } from './map.js';
...
import { getMeta, awardWin, getDoctorPromotion, promoteDoctor, isLevelCompleted } from './meta.js';
...
const levelSelectOverlay = $('levelSelectOverlay'), levelList = $('levelList');
const vResearchPoints = $('vResearchPoints'), vResearchPoints2 = $('vResearchPoints2');
const openRosterBtn = $('openRosterBtn'), closeRosterBtn = $('closeRosterBtn'), rosterFromEndBtn = $('rosterFromEndBtn');
const rosterOverlay = $('rosterOverlay'), rosterList = $('rosterList');
...
function renderLevelSelect() {
  vResearchPoints.textContent = getMeta().researchPoints;
  levelList.innerHTML = '';
  for (const level of LEVELS) {
    const card = document.createElement('div');
    card.className = 'levelCard ' + (level.implemented ? 'playable' : 'locked');
    const cleared = level.implemented && isLevelCompleted(level.id);
    card.innerHTML = `
      <div class="swatch" style="background:${hex(level.accent)}"></div>
      <div class="name">${level.name}</div>
      <div class="organ">${level.organ}</div>
      <div class="teach">${level.teach}</div>
      <span class="badge ${cleared ? 'cleared' : ''}">${level.implemented ? (cleared ? '✓ 已通關' : '可挑戰') : '開發中'}</span>
    `;
    if (level.implemented) card.addEventListener('click', () => boot(level.id));
    levelList.appendChild(card);
  }
}
renderLevelSelect();

let rosterReturnTo = 'levelSelect';
function openRoster(from) {
  rosterReturnTo = from;
  renderRoster();
...
function closeRoster() {
  ...
  if (rosterReturnTo === 'end') endOverlay.classList.remove('hidden');
  else levelSelectOverlay.classList.remove('hidden');
}
openRosterBtn.addEventListener('click', () => openRoster('levelSelect'));
rosterFromEndBtn.addEventListener('click', () => openRoster('end'));
closeRosterBtn.addEventListener('click', closeRoster);

// ==========================================================================
// Roster / promotion screen
// ==========================================================================
function renderRoster() {
  const points = getMeta().researchPoints;
  vResearchPoints.textContent = points;
  vResearchPoints2.textContent = points;
  rosterList.innerHTML = '';
  for (const def of DOCTORS) {
    const dt = damageTypeById[def.damageType];
    const promo = getDoctorPromotion(def.id);
    const tierDef = PROMOTION_TIERS[promo.tier];
    const nextTier = PROMOTION_TIERS[promo.tier + 1];
    const card = document.createElement('div');
    card.className = 'rosterCard';
    const head = document.createElement('div');
    head.className = 'rHead';
    head.innerHTML = `<div class="rSwatch" style="background:${hex(dt.color)}"></div>
      <div><div class="rName">${def.name}</div><div class="rTier">${tierDef.name}${nextTier ? ` → 下一級 ${nextTier.name}` : '（已滿級）'}</div></div>`;
    card.appendChild(head);

    if (nextTier) {
      const talentOptions = TALENTS[def.id]?.[nextTier.tier];
      const affordable = points >= nextTier.cost;
      if (talentOptions) {
        const row = document.createElement('div');
        row.className = 'talents';
        for (const t of talentOptions) {
          const btn = document.createElement('button');
          btn.className = 'talentBtn';
          btn.disabled = !affordable;
          btn.innerHTML = `<span class="tName">${t.name}（${nextTier.name}，${nextTier.cost} 點）</span><span class="tDesc">${t.desc}</span>`;
          btn.addEventListener('click', () => {
            const res = promoteDoctor(def.id, t.id);
            if (res.ok) { showToast(`${def.name} 晉升至 ${nextTier.name}：${t.name}`, 'mdt'); renderRoster(); renderLevelSelect(); }
            else showToast('研究點數不足', 'bad');
          });
          row.appendChild(btn);
        }
        ...
        btn.disabled = !affordable;
        btn.textContent = `晉升至 ${nextTier.name}（${nextTier.cost} 點）`;
        btn.addEventListener('click', () => {
          const res = promoteDoctor(def.id);
          if (res.ok) { showToast(`${def.name} 晉升至 ${nextTier.name}`, 'mdt'); renderRoster(); renderLevelSelect(); }
          else showToast('研究點數不足', 'bad');
        });
...
// ==========================================================================
// Level boot — everything below here is per-level Three.js scene state.
// Deferred until a level card is clicked so level-select never pays for a
// WebGL context or scene it might not need this session.
// ==========================================================================
let renderer, scene, camera, game;
let doctorViews, enemyViews, tileMeshes;
let selectedDoctorId = null;
let running = false;
let booted = false;

function boot(levelId) {
  if (booted) return; // one WebGL scene per page load; "restart" reloads the page
  booted = true;
  levelSelectOverlay.classList.add('hidden');

  const map = buildMap(levelId);
  ...
  for (const [x, z] of map.pathCells) { minX = Math.min(minX, x); maxX = Math.max(maxX, x); minZ = Math.min(minZ, z); maxZ = Math.max(maxZ, z); }
  for (const t of map.deployTiles) { minX = Math.min(minX, t.x); maxX = Math.max(maxX, t.x); minZ = Math.min(minZ, t.z); maxZ = Math.max(maxZ, t.z); }
  const center = gridToWorld((minX + maxX) / 2, (minZ + maxZ) / 2);
  const spanX = (maxX - minX + 4) * map.tileSize, spanZ = (maxZ - minZ + 4) * map.tileSize;
  ...
  // --- Procedural tissue floor, tinted per level -------------------------
  function buildFloorTexture() {
    const c = document.createElement('canvas');
    c.width = 64; c.height = 64;
    const ctx = c.getContext('2d');
    const base = levelById[levelId]?.accent ?? 0xc0392b;
    const br = (base >> 16) & 255, bg = (base >> 8) & 255, bb = base & 255;
    ctx.fillStyle = `rgb(${br * 0.4 | 0},${bg * 0.22 | 0},${bb * 0.24 | 0})`;
    ctx.fillRect(0, 0, 64, 64);
    for (let i = 0; i < 220; i++) {
      const x = Math.random() * 64, y = Math.random() * 64, r = Math.random() * 1.6 + 0.4;
      ctx.fillStyle = Math.random() < 0.5 ? `rgba(${br},${bg},${bb},0.35)` : `rgba(${Math.min(255, br + 90)},${Math.min(255, bg + 90)},${Math.min(255, bb + 90)},0.18)`;
  ...
  for (const [x, z] of map.pathCells) addFlatTile(x, z, map.tileSize * 0.95, 0x8a2b2b, 0.55, 0.005);
  addFlatTile(map.entrance[0], map.entrance[1], map.tileSize * 1.3, 0x35d08a, 0.75, 0.006);
  addFlatTile(map.core[0], map.core[1], map.tileSize * 1.3, 0x5ab0ff, 0.75, 0.006);

  // --- Deploy tile markers (interactive) ----------------------------------
  tileMeshes = new Map();
  const tileKey = (t) => `${t.x},${t.z}`;
  for (const tile of map.deployTiles) {
    const { mesh, mat } = addFlatTile(tile.x, tile.z, map.tileSize * 0.86, tile.type === 'ground' ? 0x2f6fb0 : 0xb08a2f, 0.14, 0.01);
    mesh.userData = { tile };
  ...
  game.on('bossSpawned', () => {
    ...
    showToast(`⚠ ${game.boss.name}出現！`, 'phase');
  });
  ...
  game.on('gameOver', ({ outcome }) => {
    running = false;
    if (outcome === 'win') {
      const points = awardWin(levelId, game.lives);
      endTitle.textContent = '手術成功 — 病患康復';
      endBody.textContent = `你成功帶領腫瘤團隊清除了所有癌細胞，並擊敗了${game.boss?.name ?? '頭目'}。獲得研究點數 +${points}。`;
    } else {
      endTitle.textContent = '病患病危 — 關卡失敗';
      endBody.textContent = `病患生命徵象歸零。再檢視一下抗性表，選對治療再試一次。`;
    }
  ...
  function renderSquadBar() {
    ...
    for (const def of DOCTORS) {
      ...
      card.innerHTML = `<div class="swatch" style="background:${hex(dt.color)}"></div>
        <div class="name">${def.name}${promo.tier ? ` ${PROMOTION_TIERS[promo.tier].name}` : ''}</div>
...
window.__oncoDefense = {
  bootLevel: (levelId) => boot(levelId),
  getGame: () => game,
};
